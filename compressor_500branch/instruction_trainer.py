...
def train(rank, args, world_size):
    ...
    #     current_model = checkpoint["model"]
    #     current_optimizer = checkpoint["optimizer"]
    #     train_examples = train_examples[current_step:]
    #     dataset = get_dataset(task_config["task_type"], train_examples, training_config['batch_size_per_device'])
    #     loader = DataLoader(dataset, batch_size=None)
    #     ddp_model.load_state_dict(current_model)
    ...
    for epoch in range(1):
        ...
        def save_checkpoint(step_num, ddp_model, optimizer):
            state = {
                "epoch": 0,
                "step": step_num,
            ...
            torch.save(state, os.path.join(args.work_dir, "checkpoint.pt"))
        if rank == 0:
        ...
        for inputs in loader:
            ...
            if step_num % (training_config["save_step"]*accumulation_steps) == 0:
                save()
            # if step_num % 100 == 0:
            #     save_checkpoint(step_num, ddp_model, optimizer)
            if rank == 0:
